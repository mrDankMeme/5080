...
@MainActor
final class AIImageResultSceneViewModel: ObservableObject {
    ...
    init(imageURL: URL) {
        ...
        let data = (try? Data(contentsOf: imageURL)) ?? Data()
    ...
    private func performSaveToGallery() async throws {
        guard let image else {
            throw APIError.backendMessage("Failed to decode image")
        }
        ...
        try await withCheckedThrowingContinuation { continuation in
            PHPhotoLibrary.shared().performChanges {
                ...
                if success {
                    ...
                    continuation.resume(throwing: error ?? APIError.backendMessage("Failed to save image"))
                }
            }
        }
    }
