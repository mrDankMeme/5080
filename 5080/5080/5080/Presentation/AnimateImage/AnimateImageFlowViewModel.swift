...
@MainActor
final class AnimateImageFlowViewModel: ObservableObject {
    enum Route {
        case composer
        case loading
        case result
    ...
    let sceneViewModel: AnimateImageSceneViewModel
    ...
    private let animateImageUseCase: AnimateImageUseCase
    ...
    init(
        sceneViewModel: AnimateImageSceneViewModel,
        ...
        animateImageUseCase: AnimateImageUseCase,
        ...
        self.animateImageUseCase = animateImageUseCase
    ...
    static var fallback: AnimateImageFlowViewModel {
        AnimateImageFlowViewModel(
            sceneViewModel: AnimateImageSceneViewModel(),
            loadingViewModel: TextToVideoLoadingSceneViewModel(title: "Animate Photo"),
            failedViewModel: TextToVideoFailedSceneViewModel(title: "Animate Photo"),
            ...
            animateImageUseCase: FallbackAnimateImageUseCase(),
    ...
    func startGeneration() {
        ...
        guard let imageData = sceneViewModel.selectedImageData else {
            sceneViewModel.showError("Please upload one image")
            return
        }
        ...
        guard generationTask == nil else {
            sceneViewModel.showAlert(
                title: HistoryFlowKind.animateImage.activeProcessingAlertTitle,
                message: HistoryFlowKind.animateImage.activeProcessingAlertMessage
            )
            return
        }
        ...
        let historyID = historyRepository.createProcessingEntry(
            flowKind: .animateImage,
            title: historyTitle(from: prompt, fallback: "Animate Photo"),
            prompt: prompt
        )
        ...
        generationTask = Task { [weak self] in
            ...
            do {
                ...
                let request = AnimateImageRequest(
                    userId: userId,
                    prompt: prompt,
                    photoURL: nil,
                    file: self.binaryUpload(from: imageData)
                )

                let startPayload = try await self.animateImageUseCase.execute(request)
    ...
    private func storeGeneratedVideo(_ data: Data) throws -> URL {
        ...
        let folderURL = cachesDirectory.appendingPathComponent("AnimateImage", isDirectory: true)
        try fileManager.createDirectory(at: folderURL, withIntermediateDirectories: true)

        let fileURL = folderURL.appendingPathComponent("animate_image_\(UUID().uuidString).mp4")
        try data.write(to: fileURL, options: .atomic)

        return fileURL
    }

    private func binaryUpload(from data: Data) -> BinaryUpload {
        let normalizedData = Self.normalizedUploadDataForTransport(from: data)
        #if DEBUG
        print("[Upload][AnimateImage] bytes=\(normalizedData.count)")
        #endif
        ...
        return BinaryUpload(
            data: normalizedData,
            fileName: "animate_\(UUID().uuidString).\(fileExtension)",
            mimeType: mimeType
        )
    }
    ...
    private func canStartAnotherGeneration() -> Bool {
        guard !historyRepository.hasProcessingEntry(flowKind: .animateImage) else {
            sceneViewModel.showAlert(
                title: HistoryFlowKind.animateImage.activeProcessingAlertTitle,
                message: HistoryFlowKind.animateImage.activeProcessingAlertMessage
            )
...
private struct FallbackFetchServicePricesUseCase: FetchServicePricesUseCase {
    func execute(userId: String) async throws -> BackendServicePricesData {
        let fallbackPayload = #"{"generate":1,"animation":8}"#
...
private struct FallbackAnimateImageUseCase: AnimateImageUseCase {
