...
struct TranscribeSettingsSheetView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            let sheetHeight = min(620.scale, proxy.size.height)
            ...
            ZStack(alignment: .bottom) {
                    ...
                    ScrollView(showsIndicators: false) {
                        VStack(alignment: .leading, spacing: 0.scale) {
                            settingsSection(
                                ...
                                options: TranscribeSceneViewModel.SourceLanguage.allCases,
                                selected: viewModel.selectedSourceLanguage,
                                onTap: {
                                    viewModel.selectedSourceLanguage = $0
                                }
                            )
                            ...
                            settingsSection(
                                title: "Output Format",
                                options: TranscribeSceneViewModel.OutputFormat.allCases,
                                selected: viewModel.selectedOutputFormat,
                                onTap: {
                                    viewModel.selectedOutputFormat = $0
                                }
                            )
                            .padding(.top, 24.scale)
                            ...
                            settingsSection(
                                title: "Timestamps",
                                options: TranscribeSceneViewModel.TimestampsMode.allCases,
                                selected: viewModel.selectedTimestampsMode,
                                onTap: {
                                    viewModel.selectedTimestampsMode = $0
                                }
                            )
                            .padding(.top, 24.scale)
                ...
                .clipShape(
                    TranscribeTopRoundedCornersShape(radius: 40.scale)
                )
...
private struct TranscribeTopRoundedCornersShape: Shape {
    let radius: CGFloat
